import { CF_ORCHESTRATOR_URL, JOB_CALLBACK_HMAC_SECRET } from '~/lib/shared/config/env'

export function requireOrchestratorUrl(): string {
	if (!CF_ORCHESTRATOR_URL)
		throw new Error('CF_ORCHESTRATOR_URL is not configured')
