...
export async function addPointsOnce(opts: {
	userId: string
	amount: number
	type: PointTransactionType
	refType?: string | null
	...
	if (!refId) {
		...
		return { credited: opts.amount, balance }
	}
	...
	if (hasInjectedD1DatabaseBinding()) {
		...
		const statements = [
			...
			d1
				...
				.bind(opts.amount, nowTs, opts.userId, opts.userId, opts.type, refId),
			d1
				.prepare(
					[
						`INSERT INTO point_transactions`,
		...
		if (txRow) {
			...
			logger.info(
				'api',
				`[points.add] user=${opts.userId} delta=${opts.amount} type=${opts.type} balance=${balance} refType=${opts.refType ?? 'null'} refId=${refId}`,
			)
			return { credited: opts.amount, balance }
		}
...
export async function getTransactionByTypeRef(opts: {
	userId: string
	type: PointTransactionType
	refId: string
	db?: DbClient
