...
function isEditingModelValid(editing: EditingModel | null): boolean {
	...
	if (editing.kind === 'asr' && editing.id.trim().startsWith('whisper/') && !editing.remoteModelId.trim()) {
		return false
	}
...
function AdminAiModelsPage() {
	...
	const providerOptions = useMemo(() => providers.filter((p) => p.kind === kind), [kind, providers])
											...
											>
												<div className="space-y-1">
													<div className="flex items-center gap-2">
														...
														{m.isDefault ? <Badge>{t('status.default')}</Badge> : null}
