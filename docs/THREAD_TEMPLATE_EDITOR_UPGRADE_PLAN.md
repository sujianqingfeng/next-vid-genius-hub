...
1) **可调布局**：三栏可拖拽调整宽度；左右面板可折叠；宽度全局记忆；折叠状态每次进入默认展开（不跨会话持久化）。
...
- **面板折叠默认展开**：左右面板允许会话内折叠，但进入页面默认展开（不跨会话持久化）。
...
- 状态包含：
  - 左/中/右栏宽度（px 或比例）
  -（注意）左/右折叠为会话内状态；进入页面默认展开（不跨会话持久化）
...
- 左/右栏可折叠；刷新页面默认展开（不记住折叠）；快捷键可用且不影响输入框。
