...
export const Route = createFileRoute("/points")({
	loader: async ({ context, location }) => {
		...
		await Promise.all([
			context.queryClient.prefetchQuery(queryOrpcNext.points.getMyBalance.queryOptions()),
...
function PointsRoute() {
	...
	const balanceQuery = useQuery(queryOrpcNext.points.getMyBalance.queryOptions())
					...
					<div className="grid gap-4 md:grid-cols-3">
						<Card className="md:col-span-2">
							...
							<CardContent>
								...
								<p className="mt-2 text-xs text-muted-foreground">{t("balance.hint")}</p>
							</CardContent>
						</Card>
						<Card>
							<CardHeader className="pb-2">
					...
					<Card>
						...
						<CardContent>
							...
							<div className="divide-y divide-border/60">
								...
								{grouped.map((item) => (
									<div key={item.id} className="grid grid-cols-5 items-center py-3 text-sm">
