...
function extractOrchestratorJobId(path: string | null | undefined): string | null {
	if (!path) return null
...
export async function resolveCloudVideoKey(
	...
	const originalJobId = extractOrchestratorJobId(input.filePath) || input.downloadJobId
