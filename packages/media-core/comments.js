...
async function getYouTubeClientForCore(proxyUrl) {
  ...
  const agent = proxyUrl ? new ProxyAgent(proxyUrl) : undefined
  const fetch = agent
    ? ((input, init) => undiciFetch(input, { ...(init || {}), dispatcher: agent }))
    : undefined
  return Innertube.create(fetch ? { cache, fetch } : { cache })
}
...
function cryptoRandomId() {
  try {
    return (globalThis.crypto?.randomUUID?.() || require('node:crypto').randomUUID?.() || Math.random().toString(36).slice(2))
  } catch {
