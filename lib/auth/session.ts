...
function buildCookie(name: string, value: string, options: { maxAge?: number; expires?: Date; path?: string }) {
...
export function createClearSessionCookie() {
	return buildCookie(SESSION_COOKIE_NAME, '', { maxAge: 0, expires: new Date(0) })
}

export async function createSession(opts: { userId: string; expiresInDays?: number; db?: DbClient }) {
	const db = opts.db ?? (await getDb())
	const token = randomBytes(32).toString('hex')
	const tokenHash = hashSessionToken(token)
	const expiresAt = new Date(Date.now() + (opts.expiresInDays ?? DEFAULT_SESSION_TTL_DAYS) * 24 * 60 * 60 * 1000)
...
export async function findSessionByToken(token: string, db?: DbClient): Promise<Session | null> {
	const client = db ?? (await getDb())
	const tokenHash = hashSessionToken(token)
