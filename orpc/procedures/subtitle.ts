...
export const transcribe = os
  ...
  .handler(async ({ input }) => {
    ...
    logger.info(
      "transcription",
      `Starting transcription for media ${mediaId} with ${provider}/${model}`,
    );
    ...
    if (provider === "cloudflare") {
      ...
      const useAsrPipeline = Boolean(
        useCloudDownsample && mediaRecord.remoteAudioKey,
      );
      ...
      logger.info(
        "transcription",
        `Using Cloudflare provider with model ${model}`,
      );
      if (useAsrPipeline) {
        // Phase 2: asr-pipeline（Worker 端降采样 + ASR）
        const targetBytes = Number(
          process.env.CLOUDFLARE_ASR_MAX_UPLOAD_BYTES || 4 * 1024 * 1024,
        );
      ...
      logger.info(
        "transcription",
        `Using local Whisper provider with model ${model}`,
      );
      const transcriptionResult = await transcribeWithWhisper({
        audioPath: hasLocalAudio
          ? mediaRecord.audioFilePath!
    ...
    if (!validation.isValid) {
      logger.warn(
        "transcription",
        `VTT format validation failed for ${provider}: ${validation.errors.join(", ")}`,
      );
      ...
      if (revalidation.isValid) {
        logger.info(
          "transcription",
          `Successfully normalized VTT format for ${provider}`,
        );
      } else {
        logger.error(
          "transcription",
          `Failed to normalize VTT format for ${provider}: ${revalidation.errors.join(", ")}`,
        );
        throw new Error(
          `Invalid VTT format from ${provider} transcription: ${revalidation.errors.join(", ")}`,
        );
      }
    }
    ...
    logger.info(
      "transcription",
      `Transcription completed successfully for media ${mediaId}`,
    );
...
- Output strictly valid JSON matching the provided schema`;
  ...
  try {
    ...
    logger.info(
      "translation",
      `Structured translation produced ${out.length} items for media ${mediaId}`,
    );
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    logger.error(
      "translation",
      `Structured translation failed for media ${mediaId}: ${msg}`,
    );
