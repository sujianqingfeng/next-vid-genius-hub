import { z } from 'zod'
import { generateObjectWithUsage } from '~/lib/features/ai/chat'
import type { AIModelId } from '~/lib/features/ai/models'
import type { TranscriptionWord } from '~/lib/infra/db/schema'
import { formatVttTimestamp } from '~/lib/features/subtitle/utils/time'
import { createVttDocument } from '~/lib/features/subtitle/utils/vtt'
...
export function buildCandidateBreaks(
	...
	for (let i = 0; i < words.length - 1; i++) {
		...
		// Break at pure-punctuation tokens or when current token ends with punctuation
		const shouldBreakByPunct =
			isPunctuation(cur.word) || endsWithPunctuation(String(cur.word))

		if (
			shouldBreakByDuration ||
			shouldBreakByChars ||
			shouldBreakByGap ||
			shouldBreakByPunct
		) {
...
function ensureCoverAllTokens(
	...
	if (cursor < wordCount)
		fixed.push({ startIndex: cursor, endIndex: wordCount - 1 })
	return fixed
}
...
export function applyPhraseGuard(
	...
	// Words that should NEVER be moved to previous sentence even if the gap is small.
	// This addresses cases like "you know", "and so", etc.
	const denyMove = new Set([
		'you',
		'we',
		'i',
		'he',
	...
	for (let i = 0; i < segments.length; i++) {
		...
		const gapMs = Math.max(
			0,
			((words[seg.startIndex]?.start ?? 0) - (words[prev.endIndex]?.end ?? 0)) *
				1000,
		)
		if (gapMs > maxGapMs) {
			out.push({ ...seg })
		...
		while (moved < maxLeadingWords && newStart <= seg.endIndex) {
			...
			const isShortNoun =
				base.length > 0 && base.length <= 12 && nouns.has(base)
			const prevLooksOpen =
				!SENTENCE_END.test(prevLast) ||
				prevLast.includes('-') ||
				/pre-[a-z]+/i.test(prevLast)
