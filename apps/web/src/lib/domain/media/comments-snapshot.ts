...
import { schema } from '~/lib/infra/db'
import type { Comment } from '~/lib/infra/db/schema'
import type { VideoInfo } from '~/lib/domain/media/types'
...
export async function buildCommentsSnapshot(
	...
	const translatedTitle =
		(options.translatedTitle ?? media.translatedTitle) || undefined
	...
	await putObjectByKey(
		key,
		'application/json',
		JSON.stringify({ videoInfo, comments: options.comments }),
	)
