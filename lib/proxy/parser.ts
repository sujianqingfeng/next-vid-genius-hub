...
export const SUPPORTED_PROXY_PROTOCOLS = ['http', 'https', 'socks4', 'socks5', 'trojan', 'vless', 'hysteria2'] as const
...
export async function parseSSRUrl(ssrUrl: string): Promise<ParsedProxy[]> {
	try {
		...
		const decodedPayload = Buffer.from(normalizedPayload, 'base64').toString('utf-8').trim()
...
function parseTrojanUrl(trojanUrl: string): ParsedProxy {
	try {
		...
		throw new Error(`Failed to parse trojan URL: ${error instanceof Error ? error.message : 'Unknown error'}`)
	}
}
...
function parseVlessUrl(vlessUrl: string): ParsedProxy {
	try {
		...
		throw new Error(`Failed to parse vless URL: ${error instanceof Error ? error.message : 'Unknown error'}`)
	}
}
...
function parseHysteria2Url(hysteriaUrl: string): ParsedProxy {
	try {
		...
		throw new Error(`Failed to parse hysteria2 URL: ${error instanceof Error ? error.message : 'Unknown error'}`)
	}
}
...
function getNodeScheme(value: string): ProxyNodeScheme | null {
	...
	for (const [scheme, prefixes] of Object.entries(NODE_SCHEME_PREFIXES) as [ProxyNodeScheme, readonly string[]][]) {
...
function decodeLineCandidate(line: string): string {
	...
	try {
		const nestedDecoded = Buffer.from(normalizeBase64(nestedCandidate), 'base64').toString('utf-8').trim()
...
function mapClashTypeToProxyProtocol(typeValue: string | undefined): ProxyProtocol | null {
...
function convertClashProxy(raw: ClashProxyRaw): ParsedProxy | null {
	...
	const name =
		toClashString(raw.name ?? raw.remark ?? raw.remarks ?? raw.title) ?? `${server}:${port}`

	let username: string | undefined
	let password: string | undefined

	const skipCertVerify = toClashBoolean(raw['skip-cert-verify'] ?? raw.skip_cert_verify ?? raw.insecure)
	const sni =
		toClashString(raw.sni ?? raw.peer ?? raw['server-name'] ?? raw.servername ?? raw.server_name) ??
	...
	const grpcServiceName = toClashString(raw['grpc-service-name'] ?? raw['service-name'] ?? raw['grpc_service_name'])
	const grpcMode = toClashString(raw['grpc-mode'] ?? raw['mode'])

	switch (protocol) {
		case 'trojan':
			username = toClashString(raw.username ?? raw.user)
			password = toClashString(raw.password ?? raw.pass ?? raw.psk ?? raw.token ?? raw.uuid ?? raw.id)
			break
		case 'vless':
			username = toClashString(raw.uuid ?? raw.id ?? raw.user ?? raw.username)
			password = toClashString(raw.password ?? raw.pass ?? raw.psk ?? raw.token ?? raw.auth)
			break
		case 'hysteria2':
			username = toClashString(raw.username ?? raw.user ?? raw.auth ?? raw.token)
			password = toClashString(raw.password ?? raw.pass ?? raw.psk ?? raw.auth ?? raw.token)
			break
		case 'http':
		case 'https':
		case 'socks5':
...
function normalizeSubscriptionPayload(rawContent: string): string {
	...
	try {
		const decoded = Buffer.from(normalizeBase64(base64Candidate), 'base64').toString('utf-8').trim()
		...
		logger.warn('proxy', `Failed to base64-decode subscription payload: ${error instanceof Error ? error.message : error}`)
	}
...
export const ProxyGroupSchema = z.object({
	...
	nodeUrl: ProxyNodeUrlSchema.optional(),
})
