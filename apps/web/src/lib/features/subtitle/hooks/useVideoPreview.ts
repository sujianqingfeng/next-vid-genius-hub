...
import type { VttCue } from '~/lib/features/subtitle/utils/vtt'
import { findActiveCue } from '~/lib/features/subtitle/utils/vtt'
...
export function useVideoPreview({
	...
	// 计算相对字体大小
	const calculateRelativeFontSize = useCallback(
		(baseFontSize: number) => {
			if (videoState.containerHeight === 0) return baseFontSize

			const scaleFactor =
				videoState.containerHeight / UI_CONSTANTS.CONTAINER_HEIGHT_REFERENCE
