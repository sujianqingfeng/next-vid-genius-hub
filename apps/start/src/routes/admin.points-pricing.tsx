...
function normalizeRule(row: unknown): PricingRule {
	...
	return {
		...
		inputPricePerUnit: typeof r.inputPricePerUnit === 'number' ? r.inputPricePerUnit : null,
		outputPricePerUnit: typeof r.outputPricePerUnit === 'number' ? r.outputPricePerUnit : null,
...
function calculateLlmPoints(opts: {
	...
	const rawMicro = BigInt(inputTokens) * inputMicro + BigInt(outputTokens) * outputMicro
	const rawPointsBig = ceilDivBigInt(rawMicro, BigInt(MICRO_POINTS_PER_POINT))
	const rawPoints =
		rawPointsBig > BigInt(Number.MAX_SAFE_INTEGER) ? Number.MAX_SAFE_INTEGER : Number(rawPointsBig)
		...
		await Promise.all([
			context.queryClient.prefetchQuery(
				queryOrpcNext.admin.listPricingRules.queryOptions({
					input: { page: 1, limit: ADMIN_PRICING_RULES_PAGE_SIZE, resourceType: 'llm' },
				}),
			),
			context.queryClient.prefetchQuery(
...
function AdminPointsPricingPage() {
	...
	const invalidate = () => {
		qc.invalidateQueries({ queryKey: queryOrpcNext.admin.listPricingRules.key() })
	}
	...
	const openEditProviderDefault = (provider: AiProvider) => {
		...
		if (kind === 'llm') {
			setEditing({
				...
				inputRmbPerMillion: rmbPerMillionTokensFromMicroPointsPerToken(effective?.inputPricePerUnit ?? 0),
				outputRmbPerMillion: rmbPerMillionTokensFromMicroPointsPerToken(
					effective?.outputPricePerUnit ?? 0,
				),
				minCharge: typeof effective?.minCharge === 'number' ? effective.minCharge : '',
			})
			return
		}
	...
	}, [globalDefaultRule, kind, modelsSorted, previewModelId, providerDefaultByProviderId, ruleByModelId])
	...
	}, [kind, previewInputTokens, previewMinutes, previewOutputTokens, previewRule, t])
		...
		return (
			<Card className="border-border/60 shadow-sm">
				<CardHeader className="flex flex-row items-center justify-between gap-3">
					...
					<Button variant="secondary" onClick={() => openEditDefault()} disabled={isBusy}>
						...
						<table className="min-w-full text-sm">
							<thead className="bg-muted/50 text-left text-xs uppercase tracking-wide text-muted-foreground">
								<tr>
									...
									{showLlmColumns ? (
										<>
											<th className="px-4 py-3 font-medium">{t('table.inputRmbPerMillion')}</th>
											<th className="px-4 py-3 font-medium">{t('table.outputRmbPerMillion')}</th>
										</>
									) : (
										<th className="px-4 py-3 font-medium">{t('table.pointsPerMinute')}</th>
									)}
							...
							<tbody className="divide-y divide-border/60">
								{rows.map((row, idx) => {
									const isProviderRow = row.modelId == null && row.provider != null
									const direct = row.direct
									const effective = row.effective

									const inputRmb =
										kind === 'llm'
											? rmbPerMillionTokensFromMicroPointsPerToken(effective?.inputPricePerUnit ?? 0)
											: null
									const outputRmb =
										kind === 'llm'
											? rmbPerMillionTokensFromMicroPointsPerToken(effective?.outputPricePerUnit ?? 0)
											: null
									...
									return (
										<tr key={row.key} className="hover:bg-muted/20">
											<td className="px-4 py-3 text-muted-foreground">{row.providerLabel}</td>
											<td className="px-4 py-3">
												<div className="font-medium text-foreground">{row.modelLabel}</div>
												{row.modelId && !isDownload ? (
													<div className="mt-0.5 text-xs text-muted-foreground">{row.modelId}</div>
												) : null}
											</td>
											{showLlmColumns ? (
												<>
													<td className="px-4 py-3 tabular-nums">{inputRmb?.toFixed(2)}</td>
													<td className="px-4 py-3 tabular-nums">{outputRmb?.toFixed(2)}</td>
												</>
											) : (
												<td className="px-4 py-3 tabular-nums">{pointsPerMinute ?? '—'}</td>
											)}
											<td className="px-4 py-3 tabular-nums">{effective?.minCharge ?? '—'}</td>
			...
			<Card className="border-border/60 shadow-sm">
				...
				<CardContent className="space-y-4">
					...
					<div className="rounded-lg border border-border/60 bg-muted/20 p-3">
						...
						<div className="mt-1 text-[10px] text-muted-foreground break-words">{preview.detail}</div>
					</div>
				</CardContent>
			</Card>
		)
										...
										<div className="space-y-2">
											...
											<Input
												...
												onChange={(e) =>
													setEditing((prev) =>
														prev && prev.mode === 'llm'
															? { ...prev, inputRmbPerMillion: Number(e.target.value || 0) }
															: prev,
													)
												}
											/>
										...
										<div className="space-y-2">
											...
											<Input
												...
												onChange={(e) =>
													setEditing((prev) =>
														prev && prev.mode === 'llm'
															? { ...prev, outputRmbPerMillion: Number(e.target.value || 0) }
															: prev,
													)
												}
											/>
								...
								<div className="space-y-2">
									...
									<Input
										...
										onChange={(e) =>
											setEditing((prev) =>
												prev && prev.mode !== 'llm'
													? { ...prev, pointsPerMinute: Number(e.target.value || 0) }
													: prev,
											)
										}
									/>
							...
							<div className="space-y-2">
								...
								<Input
									...
									onChange={(e) =>
										setEditing((prev) =>
											prev
												? { ...prev, minCharge: e.target.value === '' ? '' : Number(e.target.value) }
												: prev,
										)
									}
								/>
