import { z } from 'zod'
import type { ChatModelId } from '~/lib/features/ai/models'
import { COLOR_CONSTANTS } from '~/lib/features/subtitle/config/constants'
import type { TranscriptionLanguage } from '~/lib/features/subtitle/config/languages'
import type { WhisperModel } from '~/lib/features/subtitle/config/models'
...
export const timeSegmentEffectSchema = z
	.object({
...
export const hintTextConfigSchema = z.object({
	...
	textColor: z
		.string()
		.regex(COLOR_CONSTANTS.HEX_COLOR_REGEX, 'Invalid text color format'),
	backgroundColor: z
		.string()
		.regex(COLOR_CONSTANTS.HEX_COLOR_REGEX, 'Invalid background color format'),
	backgroundOpacity: z
		.number()
		.min(COLOR_CONSTANTS.OPACITY_MIN)
		.max(COLOR_CONSTANTS.OPACITY_MAX),
	outlineColor: z
		.string()
		.regex(COLOR_CONSTANTS.HEX_COLOR_REGEX, 'Invalid outline color format'),
...
export type { VttCue } from '~/lib/features/subtitle/utils/vtt'
