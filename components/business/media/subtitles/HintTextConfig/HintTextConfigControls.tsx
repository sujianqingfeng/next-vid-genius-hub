...
interface HintTextConfigControlsProps {
	...
	onChange: (field: keyof HintTextConfig, value: string | number | boolean) => void
}

/**
 * 提示文本配置控制组件
 */
export function HintTextConfigControls({ config, onChange }: HintTextConfigControlsProps) {
						...
						<div className="space-y-2">
							...
							<input
								...
								onChange={(e) => {
									...
									if (!Number.isNaN(value)) {
										onChange('backgroundOpacity', Math.min(Math.max(value, COLOR_CONSTANTS.OPACITY_MIN), COLOR_CONSTANTS.OPACITY_MAX))
									}
								}}
							/>
						</div>
