...
import { presignGetByKey, putObjectByKey } from '~/lib/infra/cloudflare'
import { getDb, schema, type TranscriptionWord } from '~/lib/infra/db'
...
export async function persistAsrResultFromBucket(input: PersistAsrResultInput) {
	const {
		...
		wordsKey,
		vttUrl: rawVttUrl,
		wordsUrl: rawWordsUrl,
		title,
	} = input

	const vttUrl = await resolveUrl({
		key: vttKey ?? undefined,
		url: rawVttUrl ?? undefined,
	})
	...
	try {
		const vttTargetKey = bucketPaths.inputs.subtitles(mediaId, {
