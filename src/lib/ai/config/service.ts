...
const providersCache = new Map<
	string,
	CacheEntry<(typeof schema.aiProviders.$inferSelect)[]>
>()
const modelsCache = new Map<
	string,
	CacheEntry<(typeof schema.aiModels.$inferSelect)[]>
>()
const modelByIdCache = new Map<
	string,
	CacheEntry<
...
export async function listAiModels(opts: {
	...
	if (opts.kind === 'asr') {
		...
		normalized = items.map((m) => {
			...
			if (!provider) {
				throw new Error(
					`ASR model ${m.id} references missing providerId=${m.providerId}`,
				)
			}
...
export async function getAiModelConfig(id: string, db?: DbClient) {
	...
	if (model.kind === 'asr') {
		if (provider.type === 'cloudflare_asr') {
			...
			value = {
				...model,
				capabilities: { supportsLanguageHint: true },
				provider,
			}
		} else {
			throw new Error(`Unsupported ASR provider type: ${provider.type}`)
...
export async function isEnabledModel(
	kind: AIProviderKind,
	id: string,
	db?: DbClient,
) {
