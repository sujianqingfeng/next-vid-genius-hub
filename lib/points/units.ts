...
export function rmbPerMillionTokensFromMicroPointsPerToken(microPointsPerToken: number): number {
	const micro = Number.isFinite(microPointsPerToken) ? Math.max(0, microPointsPerToken) : 0
...
export function microPointsPerTokenFromRmbPerMillionTokens(rmbPerMillionTokens: number): number {
	const rmb = Number.isFinite(rmbPerMillionTokens) ? Math.max(0, rmbPerMillionTokens) : 0
