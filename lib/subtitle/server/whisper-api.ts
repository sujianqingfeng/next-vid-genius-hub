import { createVttDocument, type VttCue } from '~/lib/subtitle/utils/vtt'
...
function extractWords(segments: WhisperSegment[]): Array<{ word: string; start: number; end: number }> | undefined {
...
export async function runWhisperApiAsr(opts: {
	...
	if (segments.length > 0) {
		return { vtt: buildVttFromSegments(segments), words: extractWords(segments) }
	}
