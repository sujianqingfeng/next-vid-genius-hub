	...
	.handler(async ({ input }) => {
		...
		try {
			...
			if (task) {
				...
				if (task.targetType === 'media' && task.targetId && status.status === 'completed') {
					const mediaId = task.targetId
					const media = await db.query.media.findFirst({ where: eq(schema.media.id, mediaId) })
					if (media) {
						const videoKey = status.outputs?.video?.key ?? status.outputKey ?? null
						...
						const metadataKey = status.outputs?.metadata?.key ?? status.outputMetadataKey ?? null

						const metadata = (status.metadata || {}) as Record<string, unknown>
						const updates: Record<string, unknown> = {}

						if (videoKey && !media.remoteVideoKey) updates.remoteVideoKey = videoKey
						if (audioProcessedKey && !media.remoteAudioKey) updates.remoteAudioKey = audioProcessedKey
						if (audioProcessedKey && !media.remoteAudioProcessedKey) updates.remoteAudioProcessedKey = audioProcessedKey
						if (audioSourceKey && !media.remoteAudioSourceKey) updates.remoteAudioSourceKey = audioSourceKey
						if (metadataKey && !media.remoteMetadataKey) updates.remoteMetadataKey = metadataKey

						const title =
							typeof metadata.title === 'string' && metadata.title.trim()
								? metadata.title.trim()
								: null
						if (title && (media.title === 'Pending download' || !media.title)) updates.title = title

						const videoBytes = typeof metadata.videoBytes === 'number' && Number.isFinite(metadata.videoBytes) ? metadata.videoBytes : null
						const audioBytes = typeof metadata.audioBytes === 'number' && Number.isFinite(metadata.audioBytes) ? metadata.audioBytes : null
						if (videoBytes !== null && (media.downloadVideoBytes == null || media.downloadVideoBytes <= 0)) {
							updates.downloadVideoBytes = videoBytes
						}
						if (audioBytes !== null && (media.downloadAudioBytes == null || media.downloadAudioBytes <= 0)) {
							updates.downloadAudioBytes = audioBytes
						}

						if (Object.keys(updates).length > 0) {
							await db.update(schema.media).set(updates).where(eq(schema.media.id, mediaId))
						}
					}
				}
			}
