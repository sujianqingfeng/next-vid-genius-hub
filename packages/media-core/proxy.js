...
export function isForwardProxyProtocolSupported(protocol) {
  return FORWARD_PROXY_PROTOCOLS.has(String(protocol || '').toLowerCase())
}

export function buildForwardProxyUrl({ protocol, server, port, username, password }) {
...
// Resolve a usable forward proxy URL from an optional explicit proxy object
// or a fallback defaultProxyUrl. Optional logger can be provided for diagnostics.
export function resolveForwardProxy({ proxy, defaultProxyUrl, logger } = {}) {
  ...
  if (proxy && proxy.server && proxy.port && proxy.protocol) {
    if (!isForwardProxyProtocolSupported(proxy.protocol)) {
      ...
      return buildForwardProxyUrl(proxy)
    }
  }
  if (defaultProxyUrl) {
