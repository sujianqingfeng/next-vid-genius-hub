...
function buildWsOpts(
	...
	const hostHeader =
		params.get('host') || params.get('authority') || opts.hostHeaderFallback
	if (hostHeader) wsOpts.headers = { Host: hostHeader }

	if (opts.includeEarlyData) {
		const earlyData =
			params.get('ed') || params.get('maxearlydata') || params.get('earlydata')
		const earlyHeader =
			params.get('edh') ||
...
export function createClashProxyFromDb(
	proxy?: {
		name?: string | null
		server?: string | null
	...
	if (nodeUrl && /^trojan:\/\//i.test(nodeUrl)) {
		try {
			...
			if (!finalPassword) {
				console.warn(
					'[media-core/clash] Trojan node missing password, skipping',
					baseName,
				)
				return null
			}
	...
	if (nodeUrl && /^vless:\/\//i.test(nodeUrl)) {
		try {
			...
			if (!uuid) {
				console.warn(
					'[media-core/clash] VLESS node missing UUID, skipping',
					baseName,
				)
				return null
			}
			...
			const sni = params.get('sni') || params.get('host') || hostname
			...
			if (security === 'tls' || security === 'reality') {
				...
				if (security === 'reality') {
					...
					const shortId =
						params.get('sid') || params.get('shortid') || params.get('short-id')
					const spiderX =
						params.get('spx') || params.get('spiderx') || params.get('spider-x')
	...
	if (proxy.server && proxy.port && proxy.protocol) {
		...
		if (proxy.protocol === 'trojan') {
			...
			if (!finalPassword) {
				console.warn(
					'[media-core/clash] Trojan proxy missing password; cannot configure Clash',
...
export function buildClashConfig(
	...
	if (!proxies.length && !subscriptionUrl) {
		const proxyDebug = engineOptions?.proxy
			...
			: null
		logger?.log?.(
			'[media-core/clash] skipping mihomo (no proxy config available)',
			{
		...
		const attempt = () => {
			...
			socket.on('error', (error) => {
				...
				if (attempts >= maxAttempts) {
					reject(
						new Error(
							`Clash proxy not ready on ${host}:${port}: ${(error as any).message}`,
						),
					)
				} else {
