...
import { UI_CONSTANTS } from '~/lib/features/subtitle/config/constants'
import type { SubtitleRenderConfig, VttCue } from '~/lib/features/subtitle/types'
import { hexToRgba } from '~/lib/shared/utils/format/color'
...
export function SubtitleOverlay({
	...
	const baseFontSize = containerHeight
		? (config.fontSize / UI_CONSTANTS.CONTAINER_HEIGHT_REFERENCE) *
			containerHeight
		: config.fontSize

	const chineseFontSize = Math.max(
		baseFontSize,
		UI_CONSTANTS.MIN_CHINESE_FONT_SIZE,
	)
	...
	// 计算文本阴影和背景色
	const textShadowBlur = Math.max(
		chineseFontSize * UI_CONSTANTS.TEXT_SHADOW_BLUR_RATIO,
		UI_CONSTANTS.MIN_TEXT_SHADOW_BLUR,
	)
