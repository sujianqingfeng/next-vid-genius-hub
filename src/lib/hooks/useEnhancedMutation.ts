...
export type MaybeMessage<TPayload> =
	| string
	| ((payload: TPayload) => string | undefined | null)
...
export interface UseEnhancedMutationSideEffects<
	TData,
	TError,
	TVariables,
	TContext,
...
function resolveMessage<TPayload>(
	message: MaybeMessage<TPayload> | undefined,
	payload: TPayload,
) {
...
export function useEnhancedMutation<TData, TError, TVariables, TContext>(
	...
	sideEffects: UseEnhancedMutationSideEffects<
		TData,
		TError,
		TVariables,
		TContext
	...
	return useMutation({
		...
		onSuccess: (data, variables, context, mutation) => {
			...
			const message = resolveMessage(sideEffects.successToast, {
				data,
				variables,
				context,
			})
		...
		onError: (error, variables, context, mutation) => {
			...
			const message = resolveMessage(sideEffects.errorToast, {
				error,
				variables,
				context,
			})
