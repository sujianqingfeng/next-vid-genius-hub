...
function mediaPreviewUrl(media: MediaItem, id: string): string | null {
	if (media.videoWithInfoPath) return `/api/media/${encodeURIComponent(id)}/rendered-info`
	if (media.videoWithSubtitlesPath) return `/api/media/${encodeURIComponent(id)}/rendered`
...
function MediaDetailIndexRoute() {
	...
	React.useEffect(() => {
		setSelectedProxyId((cur) => {
			...
			if (defaultProxyId && proxies.some((p) => p.id === defaultProxyId)) return defaultProxyId
			return cur
		})
				...
				<div className="mx-auto max-w-6xl space-y-6">
					<div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
						<div className="space-y-2">
							...
							{item.translatedTitle ? (
								<div className="text-base text-muted-foreground">{item.translatedTitle}</div>
							) : null}
					...
					<div className="grid grid-cols-1 gap-6 lg:grid-cols-[2fr_1fr]">
						...
						<div className="space-y-4">
							...
							<Card>
								<CardHeader>
									<CardTitle className="text-base">{t("actions.sync")}</CardTitle>
								</CardHeader>
								<CardContent className="space-y-4">
									<div className="space-y-2">
										...
										<Select
											...
											disabled={refreshMutation.isPending || proxiesQuery.isLoading}
										>
