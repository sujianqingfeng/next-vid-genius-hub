...
import { extractTikTokVideoId, isTikTokUrl } from './utils'
...
export const tiktokProvider: VideoProvider = {
	...
	async fetchMetadata(
		...
		try {
			...
			logger.error(
				'media',
				`TikTok metadata fetch error: ${error instanceof Error ? error.message : String(error)}`,
			)
			throw new Error(
				`TikTok metadata fetch failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
			)
		}
	},
	...
	async getThumbnailUrl(
		videoId: string,
		_quality: 'default' | 'medium' | 'high' = 'default',
	): Promise<string> {
	...
	> {
		try {
			...
			logger.error(
				'media',
				`Failed to fetch trending TikTok videos: ${error instanceof Error ? error.message : String(error)}`,
			)
			return []
		}
	},
	...
	> {
		try {
			...
			logger.error(
				'media',
				`Failed to fetch user TikTok videos: ${error instanceof Error ? error.message : String(error)}`,
			)
			return []
		}
	},
	...
	> {
		try {
			...
			logger.error(
				'media',
				`Failed to search TikTok videos: ${error instanceof Error ? error.message : String(error)}`,
			)
			return []
		}
	},
	...
	> {
		try {
			...
			const comments = await downloadTikTokCommentsByUrl(
				url,
			...
			logger.error(
				'media',
				`Failed to fetch TikTok comments: ${error instanceof Error ? error.message : String(error)}`,
			)
			return []
		}
	},
