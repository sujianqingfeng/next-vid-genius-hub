...
export function TimelineSelector({
	...
	// 处理鼠标按下事件
	const handleMouseDown = (e: React.MouseEvent, type: 'start' | 'end' | 'segment') => {
		e.preventDefault()
	...
	useEffect(() => {
		...
		const handleMouseMove = (e: MouseEvent) => {
			...
			switch (isDragging) {
				case 'start':
					newStartTime = Math.max(0, Math.min(dragEndTime - 0.5, dragStartTime + deltaTime))
					break
				case 'end':
					newEndTime = Math.min(duration, Math.max(dragStartTime + 0.5, dragEndTime + deltaTime))
					break
				case 'segment':
					const segmentDelta = deltaTime
					if (dragStartTime + segmentDelta >= 0 && dragEndTime + segmentDelta <= duration) {
	...
	const handleTimelineClick = (e: React.MouseEvent) => {
		...
		if (clickTime >= startTime && clickTime <= endTime) {
			...
			const newStartTime = Math.max(0, Math.min(duration - segmentDuration, clickTime - segmentDuration / 2))
	...
	return (
		<div className="space-y-4">
			...
			<div className="text-sm text-gray-600 dark:text-gray-400">
				Duration: <span className="font-medium">{formatTimeForDisplay(endTime - startTime)}</span>
				{conflicts.length > 0 && (
					<div className="text-red-600 dark:text-red-400 mt-1">
						⚠️ Conflicts with existing segments. Adjust the time range to avoid conflicts.
					</div>
				)}
			</div>
		</div>
